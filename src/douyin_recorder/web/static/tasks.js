...
function render() {
  ...
  if (state.tasks.length === 0) {
    ...
    setTextIfChanged(elements.emptyDetail, "粘贴抖音直播间链接或分享文案，创建第一个任务");
  } else {
...
async function inspectRoom() {
  ...
  renderInspectResult({ tone: "loading", glyph: "clock", title: "正在检测直播间", detail: "解析分享链接并读取直播状态…" });
  try {
    ...
    renderInspectResult({
      ...
      detail: [result.is_live ? "直播中" : "当前未开播", result.title, sources && `可用源 ${sources}`]
        .filter(Boolean)
        .join(" · "),
    });
  } catch (error) {
